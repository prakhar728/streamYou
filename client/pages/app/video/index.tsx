...
const room = () => {
  ...
  const { startRecording, stopRecording, isStarting, inProgress, isStopping, error: RecordingError,data: recordingData, } = useRecording();
      ...
      <Navbar>
        <Box border="1px" borderColor={"black"} h='80vh'>
          <Box h='100%' >
            ...
            <Stack border="1px" borderColor={"black"} h='20%' columnGap={"2vw"} flexDirection={'row'} alignItems={'center'} justifyContent={'space-evenly'} flexWrap={'wrap'} >
              ...
              <Button isDisabled={!fetchAudioStream.isCallable} onClick={fetchAudioStream} colorScheme='blue'>
                FETCH_AUDIO_STREAM
              </Button>

              <Button isDisabled={fetchAudioStream.isCallable} onClick={stopAudioStream} colorScheme='blue'>
                STOP AUDIO STREAM
              </Button>
              {/* Webcam */}
              <Button isDisabled={!fetchVideoStream.isCallable} onClick={fetchVideoStream} colorScheme='blue'>
                FETCH_VIDEO_STREAM
              </Button>
              <Button isDisabled={!stopVideoStream.isCallable} onClick={stopVideoStream} colorScheme='blue'>
                STOP VIDEO STREAM
              </Button>
              <Button colorScheme='blue' isDisabled={!produceVideo.isCallable} onClick={() => produceVideo(videoStream)}>
                Produce Cam
              </Button>

              <Button colorScheme='blue' isDisabled={!produceAudio.isCallable} onClick={() => produceAudio(audioStream)}>
                Produce Mic
              </Button>

              <Button colorScheme='blue' isDisabled={!stopProducingVideo.isCallable} onClick={stopProducingVideo}>
                Stop Producing Cam
              </Button>

              <Button colorScheme='blue' isDisabled={!stopProducingAudio.isCallable} onClick={stopProducingAudio}>
                Stop Producing Mic
              </Button>
